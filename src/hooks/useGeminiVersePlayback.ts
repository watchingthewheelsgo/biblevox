...
import type { GeminiVoiceGender } from "@/lib/geminiVoicePreference";
...
export function useGeminiVersePlayback(
  bookId: string,
  chapterNum: number,
  ...
  const playOneVerse = useCallback(
    (verseText: string, verseNum: number): Promise<void> => {
      if (!enabled || cancelledRef.current) return Promise.resolve();
      return fetch("/api/tts/gemini", {
        method: "POST",
    ...
    [bookId, chapterNum, gender, speed, enabled],
  );
  ...
  const playFromVerse = useCallback(
    ...
    [chapter, enabled, playOneVerse, stopInternal],
  );
