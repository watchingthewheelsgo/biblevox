...
const FEATURED_READINGS = [
  { title: "Psalm 23", subtitleKey: "listen.featured.ps23.subtitle", book: "Psalms", link: "/read/psalms/23", color: "from-amber-900/25" },
  { title: "Genesis 1", subtitleKey: "listen.featured.gen1.subtitle", book: "Genesis", link: "/read/genesis/1", color: "from-blue-900/25" },
  { title: "John 1", subtitleKey: "listen.featured.john1.subtitle", book: "John", link: "/read/john/1", color: "from-emerald-900/25" },
  { title: "Romans 8", subtitleKey: "listen.featured.rom8.subtitle", book: "Romans", link: "/read/romans/8", color: "from-purple-900/25" },
  { title: "Isaiah 53", subtitleKey: "listen.featured.isa53.subtitle", book: "Isaiah", link: "/read/isaiah/53", color: "from-red-900/25" },
  { title: "Revelation 21", subtitleKey: "listen.featured.rev21.subtitle", book: "Revelation", link: "/read/revelation/21", color: "from-indigo-900/25" },
];

const PLAYLISTS = [
  {
    titleKey: "listen.playlist.gospels.title",
    descriptionKey: "listen.playlist.gospels.desc",
    chapters: [
  ...
  {
    titleKey: "listen.playlist.psalms.title",
    descriptionKey: "listen.playlist.psalms.desc",
    chapters: [
  ...
  {
    titleKey: "listen.playlist.wisdom.title",
    descriptionKey: "listen.playlist.wisdom.desc",
    chapters: [
    ...
    <div className="max-w-2xl mx-auto px-5 pt-10 pb-8 space-y-10">
      ...
      <header className="animate-fade-in-up">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">{t("listen.title")}</h1>
        </div>
          ...
          {FEATURED_READINGS.map((item) => (
            <Link key={item.link} to={item.link} className="group">
              <div className={`relative overflow-hidden glass rounded-xl p-4 h-full hover:border-border-accent transition-all duration-300 bg-gradient-to-br ${item.color} to-transparent`}>
                ...
                <p className="text-[10px] text-text-secondary">{t(item.subtitleKey)}</p>
      ...
      <section className="animate-fade-in-up delay-200">
        ...
        <div className="space-y-3">
          {PLAYLISTS.map((playlist) => (
            <div key={playlist.titleKey} className="glass rounded-xl p-4">
              <div className="flex items-start gap-3 mb-3">
                <BookOpenIcon className="w-5 h-5 text-accent/40 mt-0.5" />
                <div>
                  <h3 className="text-[13px] font-semibold text-text-primary">{t(playlist.titleKey)}</h3>
                  <p className="text-[11px] text-text-secondary">{t(playlist.descriptionKey)}</p>
                </div>
              </div>
      ...
      <section className="animate-fade-in-up delay-300">
        <Link to="/books" className="group block">
          <div className="glass rounded-xl p-5 text-center hover:border-border-accent transition-all duration-300">
            ...
            <p className="text-sm font-medium text-text-primary mb-0.5">{t("listen.browseAll")}</p>
            <p className="text-[11px] text-text-secondary flex items-center justify-center gap-1">
              {t("listen.booksAndChapters")} <ChevronRightIcon className="w-3 h-3" />
            </p>
          </div>
        </Link>
      </section>
